...
class TestAuth:
    def test_authenticated_request(self, auth_api_client, user, user_profile):
        response = auth_api_client.get('/api/v1/user/me/')
    ...
    def test_login(self, api_client, user, token):
        ...
        response = api_client.post('/api/v1/user/auth/login/', payload, format='json')
    ...
    def test_logout(self, auth_api_client):
        response = auth_api_client.post('/api/v1/user/auth/logout/')
    ...
    def test_register(self, api_client):
        ...
        response = api_client.post('/api/v1/user/auth/register/', payload, format='json')
